...
- **Трек A (.NET)** — 11 тасок: разобраться в коде → оживить CI → починить поток регистрации в .NET → построить две фичи → покрыть e2e-тестами.
- **Трек B (TypeScript)** — 9 тасок: разобраться в коде → оживить CI → мигрировать бэкенд на NestJS + Prisma, починив найденные дефекты переносом → построить те же две фичи на новом бэкенде → покрыть e2e-тестами.
...
Текст каждой таски лежит отдельным файлом в [`tasks/`](tasks/). Начиная сессию, давайте агенту прочитать именно файл текущей таски — а не весь PRODUCT.md: агенту, как и вам, лишний контекст только мешает.
...
- **Свободный режим** (discovery, EXT-110, фиксы Phase 1A, BEVN-101, фронтенд-фикс) — работайте с агентом так, как вам удобно.
